...
print "pos_specify is :@pos_specify\n";
my ($chr_spec, $start_spec, $end_spec) = @pos_specify;
...
### draw genome line
			...
			foreach my $kk(keys %{$scaffolds{$scf}{'gaps'}}){
				...
				die "error:test\n";
				#next if($);
				$start = $start - $start_spec +1;
				$end = $end - $start_spec +1;
...
### draw PE half circos		
		foreach my $flag(sort {$lib{$scf}{$a}{'start1'}<=>$lib{$scf}{$b}{'start1'}} keys %{$lib{$scf}}){
			...
			next unless($s1>=$start_spec && ($s2+$length) <=$end_spec);# filter reads which are not in the specific region
